...
El schedule lo gestiona GitHub Actions (monitor.yml)
"""
...
def main():
    print(f"[{datetime.now().strftime('%d/%m/%Y %H:%M:%S')}] Comprobando página...")

    ultimo_valor = cargar_ultimo_valor()
    valor_actual = obtener_valor_pagina()

    if valor_actual is None:
        print("No se pudo obtener el valor de la página.")
        return
    ...
    if valor_actual != ultimo_valor:
        ...
        # GitHub Actions enviará el email via step separado
        # Guardamos el nuevo valor
        guardar_valor(valor_actual)
        # Escribimos variable de entorno para que el workflow sepa que hubo cambio
